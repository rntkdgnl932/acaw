    ...
    try:
        ...
        while clean_ is False:
            ...
            if result_out == True:
                ...
                # 일반적인 뒤로가기
                full_path = "c:\\acaw\\my_acaw\\imgs\\clean_screen\\back_1.PNG"
                ...
                if imgs_ is not None and imgs_ != False:
                    print("bag_1", imgs_)
                    full_path = "c:\\acaw\\my_acaw\\imgs\\clean_screen\\close_to_potion.PNG"
                    img_array = np.fromfile(full_path, np.uint8)
                    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                    ...
                    if imgs_ is not None and imgs_ != False:
                        ...
                        click_pos_reg(imgs_.x, imgs_.y, cla)
                # 기술 닫기
                full_path = "c:\\acaw\\my_acaw\\imgs\\clean_screen\\skill.PNG"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                imgs_ = imgs_set_(690, 650, 750, 710, cla, img, 0.8)
                if imgs_ is not None and imgs_ != False:
                    ...
                    if imgs_ is not None and imgs_ != False:
                        ...
                        click_pos_reg(imgs_.x, imgs_.y, cla)
                # 메뉴 닫기
                full_path = "c:\\acaw\\my_acaw\\imgs\\clean_screen\\setting.PNG"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                imgs_ = imgs_set_(790, 300, 840, 350, cla, img, 0.8)
                if imgs_ is not None and imgs_ != False:
                    print("setting", imgs_)
                    click_pos_2(900, 55, cla)
                # 커뮤니티 닫기
                full_path = "c:\\acaw\\my_acaw\\imgs\\clean_screen\\community.PNG"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                ...
                # 공간이동서 닫기
                full_path = "c:\\acaw\\my_acaw\\imgs\\clean_screen\\gongan_move.PNG"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                ...
                # 건너뛰기 닫기
                full_path = "c:\\acaw\\my_acaw\\imgs\\clean_screen\\skip.PNG"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
...
def out_check(cla):
    try:
        ...
        full_path = "c:\\acaw\\my_acaw\\imgs\\check\\out_check_1.PNG"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
