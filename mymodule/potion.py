...
def my_potion_check(cla):
    try:
        import cv2
        import numpy as np
        from function import click_pos_reg, imgs_set_, text_check_get, click_pos_2, drag_pos, in_number_check, int_put_, change_number
        ...
        while potion_read_ready is False:
            ...
            full_path = "c:\\acaw\\my_acaw\\imgs\\potion\\potion_1.PNG"
                ...
                if result_out == True:
                    ...
                    if imgs_ is not None and imgs_ != False:
                        ...
                        while look_potion is False:
                            ...
                            full_path = "c:\\acaw\\my_acaw\\imgs\\potion\\small_potion_11.PNG"
                            img_array = np.fromfile(full_path, np.uint8)
                            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                            ...
                            if imgs_ is not None and imgs_ != False:
                                ...
                                full_path = "c:\\acaw\\my_acaw\\imgs\\potion\\small_potion_1.PNG"
                                img_array = np.fromfile(full_path, np.uint8)
                                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                                ...
                                if imgs_ is not None and imgs_ != False:
                                    ...
                                    full_path = "c:\\acaw\\my_acaw\\imgs\\potion\\small_potion_2.PNG"
                                    img_array = np.fromfile(full_path, np.uint8)
                                    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                                    imgs_ = imgs_set_(695, 355, 940, 670, cla, img, 0.9)
                                    if imgs_ is not None and imgs_ != False:
                        ...
                        if look_ok == True:
                            ...
                            full_path = "c:\\acaw\\my_acaw\\imgs\\potion\\zero.PNG"
                            img_array = np.fromfile(full_path, np.uint8)
                            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
...
def maul_move_check(cla):
    try:
        ...
        full_path = "c:\\acaw\\my_acaw\\imgs\\common_grow\\maul_move_check.PNG"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        ...
        if imgs_ is not None and imgs_ != False:
            ...
            while search_maul_move is False:
                ...
                time.sleep(0.5)
                full_path = "c:\\acaw\\my_acaw\\imgs\\common_grow\\maul_move_check2.PNG"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
...
def soongan_move_check(cla):
    try:
        ...
        full_path = "c:\\acaw\\my_acaw\\imgs\\common_grow\\soongan_move_check.PNG"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        ...
        if imgs_ is not None and imgs_ != False:
            ...
            while search_maul_move is False:
                ...
                click_pos_2(815, 680, cla)
                time.sleep(0.5)
                full_path = "c:\\acaw\\my_acaw\\imgs\\common_grow\\soongan_move_check2.PNG"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                imgs_ = imgs_set_(680, 350, 940, 670, cla, img, 0.8)
                if imgs_ is not None and imgs_ != False:
...
def buy_potion(cla):
    try:
        import cv2
        import numpy as np
        from function import click_pos_reg, imgs_set_, click_pos_2, in_number_check, int_put_, text_check_get, change_number
        ...
        while close_to_potion is False:
            ...
            full_path = "c:\\acaw\\my_acaw\\imgs\\potion\\close_to_potion.PNG"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        ...
        while in_maul is False:
            full_path = "c:\\acaw\\my_acaw\\imgs\\potion\\maul_.PNG"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            ...
            if imgs_ is not None and imgs_ != False:
                ...
                in_maul = True
                full_path = "c:\\acaw\\my_acaw\\imgs\\potion\\jabhwa_1.PNG"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                ...
                if imgs_ is not None and imgs_ != False:
                    ...
                    full_path = "c:\\acaw\\my_acaw\\imgs\\potion\\jabhwa_2.PNG"
                    img_array = np.fromfile(full_path, np.uint8)
                    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                    imgs_ = imgs_set_(20, 80, 300, 200, cla, img, 0.8)
                    if imgs_ is not None and imgs_ != False:
                ...
                full_path = "c:\\acaw\\my_acaw\\imgs\\potion\\jabhwa_title.PNG"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        ...
        if again == False:
            ...
            while isJabhwa is False:
                ...
                if imgs_ is not None and imgs_ != False:
                    print("jabhwa_title", imgs_)
                    # 포션 구매
                    full_path = "c:\\acaw\\my_acaw\\imgs\\potion\\40level.PNG"
                    img_array = np.fromfile(full_path, np.uint8)
                    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                    ...
                    if imgs_ is not None and imgs_ != False:
                        print("40level", imgs_)
                        print("작은 포션 사자")
                        full_path = "c:\\acaw\\my_acaw\\imgs\\potion\\jabhwa_small_potion.PNG"
                        img_array = np.fromfile(full_path, np.uint8)
                        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                        imgs_ = imgs_set_(0, 140, 240, 970, cla, img, 0.8)
                        if imgs_ is not None and imgs_ != False:
                            ...
                            if v_.now_ing_schedule != "튜토육성":
                                ...
                                while per_ready is False:
                                    for i in range(3):
                                        ...
                                        buy_ready = in_number_check(cla, potion_per)
                                        if buy_ready == True:
                                            break
                                    if buy_ready == True:
                                            ...
                                            if imgs_ is not None and imgs_ != False:
                                                ...
                                                if click_count > 0:
                                                    ...
                                                    while last_buy is False:
                                                        full_path = "c:\\acaw\\my_acaw\\imgs\\potion\\jab_confirm.PNG"
                                                        img_array = np.fromfile(full_path, np.uint8)
                                                        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                    ...
                    while move_ is False:
                        ...
                        if imgs_ is not None and imgs_ != False:
                            ...
                            full_path = "c:\\acaw\\my_acaw\\imgs\\potion\\guihwan_1.PNG"
                            img_array = np.fromfile(full_path, np.uint8)
                            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                            imgs_ = imgs_set_(0, 140, 240, 970, cla, img, 0.8)
                            if imgs_ is not None and imgs_ != False:
                    ...
                    while move_ is False:
                        ...
                        if imgs_ is not None and imgs_ != False:
                            ...
                            full_path = "c:\\acaw\\my_acaw\\imgs\\potion\\soongan_1.PNG"
                            img_array = np.fromfile(full_path, np.uint8)
                            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                            imgs_ = imgs_set_(0, 140, 240, 970, cla, img, 0.8)
                            if imgs_ is not None and imgs_ != False:
...
def grow_jangbi_check(cla):
    try:
        ...
        for i in range(len(jb_list)):
            ...
            full_path = "c:\\acaw\\my_acaw\\imgs\\common_grow\\" + jb_list[i] + ".PNG"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            ...
            if imgs_ is not None and imgs_ != False:
                ...
                x_reg = imgs_.x
                y_reg = imgs_.y
                full_path = "c:\\acaw\\my_acaw\\imgs\\common_grow\\jangchag.PNG"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
