...
from tuto_grow import tuto_start
from jadong import jadong_start
from common_grow import common_start
...
class MyApp(QDialog):
    ...
    def initUI(self):
        ...
        self.setWindowTitle("아키에이지(ver " + version + ")")
        # 풀버젼
        # pyinstaller --hidden-import PyQt5 --hidden-import requests --hidden-import chardet --add-data="C:\\acaw\\my_acaw;./my_acaw" --add-data="C:\\acaw\\mysettings;./mysettings" -i="acaw.ico" --add-data="acaw.ico;./" --icon="acaw.ico" --paths "C:\Users\1_S_3\AppData\Local\Programs\Python\Python311\Lib\site-packages\cv2" main.py
        # 업데이트버젼
        # pyinstaller --hidden-import PyQt5 --hidden-import requests --hidden-import chardet --add-data="C:\\acaw\\my_acaw;./my_acaw" -i="acaw.ico" --add-data="acaw.ico;./" --icon="acaw.ico" --paths "C:\Users\1_S_3\AppData\Local\Programs\Python\Python311\Lib\site-packages\cv2" main.py

        self.setGeometry(10, 300, 900, 600)
        self.show()
...
class SecondTab(QWidget):
    ...
    def win_left(self):
        print("왼쪽으로 정렬 합니다.")
        full_path = "c:\\acaw\\my_acaw\\imgs\\monitoring\\acaw_title.PNG"
...
class FirstTab(QWidget):
    ...
    def initUI(self):
        ...
        file_path5 = dir_path + "\\my_acaw\\jadong\\jadong_force_list.txt"
        ...
        while j_l_ is False:
            if os.path.isfile(file_path5) == True:
                ...
                with open(file_path5, "w", encoding='UTF8') as file:
                    file.write("백월만")
        ...
        cb6 = QComboBox()
        list6 = ['스케쥴 선택', '튜토육성', '일반육성', '각종템받기', '거래소등록']
        cb6.addItems(list6)
        vbox6 = QHBoxLayout()
        vbox6.addWidget(cb6)
        ...
        # 사냥터
        dir_path = "C:\\acaw\\my_acaw"
        file_path1 = dir_path + "\\jadong\\jadong.txt"
        ...
        if os.path.isfile(file_path1) == True:
            with open(file_path1, "r", encoding='UTF8') as file:
                ...
                add_title = "자동사냥터"
                list5 = []
                list5.append(add_title)
                for i in range(len(read_1)):
                    jadong_list_result = read_1[i].split("/")
                    list5.append(jadong_list_result[0])
            # with open(file_path2, "r", encoding='UTF8') as file:
            #     read_1 = file.read()
            #     read_1 = read_1.split(":")
    ...
    def onActivated_slelect_spot_read(self):
        dir_path = "C:\\acaw"
        dir_spot = "C:\\acaw\\mysettings\\gold_force"
        ...
        jadong = dir_path + "\\my_acaw\\jadong\\jadong.txt"
        # bastium = dir_path + "\\my_acaw\\jadong\\bastium.txt"
        # chalano = dir_path + "\\my_acaw\\jadong\\chalano.txt"

        spot_ = "none"

        jadong_list_force = v_.onForceGoldSpot
        print("jadong_list_force", jadong_list_force)
        with open(jadong, "r", encoding='UTF8') as file:
            jadong_list = file.read().splitlines()
            for i in range(len(jadong_list)):
                jadong_list_result = jadong_list[i].split("/")
                if jadong_list_force == jadong_list_result[0]:
                    spot_ = "사냥_" + jadong_list_force
                    print("spot_1", spot_)
    ...
    def onActivated_slelect_gold_read(self):
        dir_path = "C:\\acaw"
        dir_gold = "C:\\acaw\\mysettings\\gold_force"
    ...
    def onActivated_slelect_collection_toggle_read(self):
        ...
        dir_path = "C:\\acaw"
        dir_toggle = "C:\\acaw\\mysettings\\collection"
    ...
    def onActivated_hunt_add(self):
        ...
        if onCharacter == 0:
            pyautogui.alert(button='넵', text='캐릭터를 선택해 주시지예', title='뭐합니꺼')
        elif onHunt == '자동사냥터' or onHunt == 'none':
            pyautogui.alert(button='넵', text='던전을 선택해 주시지예', title='뭐합니꺼')
        elif onCharacter != 0 and onHunt != '자동사냥터':
            print('char_', char_)
            print('dun_', hun_)
...
class game_Playing(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            print("acaw\\my_acaw go", v_.now_cla)

            while self.isCheck is True:

                print("아키에이지 워 실행 모드(ver " + version + ")")

                v_.grow_jangbi = False

                full_path = "c:\\acaw\\my_acaw\\imgs\\check\\touching.PNG"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                ...
                if imgs_ is not None and imgs_ != False:
                    ...
                    if "_" in result_schedule_:
                        ...
                        if result_schedule_ == "일반육성":
                            v_.grow_jangbi = True
                            common_start(v_.now_cla, result_schedule_)
