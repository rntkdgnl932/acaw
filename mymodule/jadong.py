...
def in_spot_check(cla):
    try:
        ...
        in_spot_ = False
        full_path = "c:\\acaw\\my_acaw\\imgs\\jadong\\jadong_list_in\\common.PNG"
...
def in_worldmap(cla):
    try:
        ...
        while in_w_ is False:
            ...
            if in_w_count > 10:
                in_w_ = True
            full_path = "c:\\acaw\\my_acaw\\imgs\\jadong\\worldmap_title.PNG"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
    ...
    try:
        ...
        if imgs_ is not None and imgs_ != False:
            ...
            while jadong_hunt is False:
                ...
                full_path = "c:\\acaw\\my_acaw\\imgs\\jadong\\jadong_soongan_move.PNG"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                ...
                if imgs_ is not None and imgs_ != False:
                    ...
                    click_pos_2(30, 180, cla)
                    full_path = "c:\\acaw\\my_acaw\\imgs\\jadong\\jadong_list\\" + spot_name + ".PNG"
                    img_array = np.fromfile(full_path, np.uint8)
                    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                ...
                full_path = "c:\\acaw\\my_acaw\\imgs\\jadong\\confirm_1.PNG"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
