...
from function_acaw import imgs_set, imgs_set_, click_pos_2, random_int, text_check_get_3, int_put_, text_check_get, click_with_image, drag_pos, image_processing, get_region, click_pos_reg, win_left_move, win_right_move
...
class MyApp(QDialog):
    ...
    def initUI(self):
        ...
        # 풀버젼
        # pyinstaller --hidden-import PyQt5 --hidden-import pyserial --hidden-import requests --hidden-import chardet --add-data="C:\\my_games\\acaw\\data_acaw;./data_acaw" --add-data="C:\\my_games\\acaw\\mysettings;./mysettings" --name acaw -i="acaw_macro.ico" --add-data="acaw_macro.ico;./" --icon="acaw_macro.ico" --paths "C:\Users\1_S_3\AppData\Local\Programs\Python\Python311\Lib\site-packages\cv2" main.py
        # 업데이트버젼
...
class FirstTab(QWidget):
    ...
    def onActivated_dunjeon_2_add(self):
        char_ = onCharacter
        dun_ = "바다_" + str(onDunjeon_2) + "_" + str(onDunjeon_2_level)
        if onCharacter == 0:
            pyautogui.alert(button='넵', text='캐릭터를 선택해 주시지예', title='뭐합니꺼')
        elif onCla == 'none':
            pyautogui.alert(button='넵', text='몇 클라인지 선택해 주시지예', title='뭐합니꺼')
        elif onDunjeon_2 == '바다 선택' or onDunjeon_2 == 'none' or onDunjeon_2_level == 0 or onDunjeon_2_level == "층":
            pyautogui.alert(button='넵', text='던전 및 층수를 선택해 주시지예', title='아 진짜 뭐합니꺼')
        elif onCharacter != 0 and (onDunjeon_2 != '바다 선택' or onDunjeon_2 != 'none'):
            print('char_', char_)
            print('dun_', dun_)
    ...
    def onActivated_dunjeon_3_add(self):
        char_ = onCharacter
        dun_ = "렐름_" + str(onDunjeon_3) + "_" + str(onDunjeon_3_level)
        if onCharacter == 0:
            pyautogui.alert(button='넵', text='캐릭터를 선택해 주시지예', title='뭐합니꺼')
        elif onCla == 'none':
            pyautogui.alert(button='넵', text='몇 클라인지 선택해 주시지예', title='뭐합니꺼')
        elif onDunjeon_3 == '렐름 선택' or onDunjeon_3 == 'none' or onDunjeon_3_level == 0 or onDunjeon_3_level == "층":
            pyautogui.alert(button='넵', text='던전 및 층수를 선택해 주시지예', title='아 진짜 뭐합니꺼')
        elif onCharacter != 0 and (onDunjeon_2 != '렐름 선택' or onDunjeon_2 != 'none'):
            print('char_', char_)
            print('dun_', dun_)
                    ...
                    if imgs_ is not None and imgs_ != False:
                        ...
                        if game_title == True:
                            ...
                            if look_title == True:
                                ...
                                elif "바다" in result_schedule_:
                                    dungeon_start(v_.now_cla, result_schedule_)
                                elif "렐름" in result_schedule_:
                                    dungeon_start(v_.now_cla, result_schedule_)
